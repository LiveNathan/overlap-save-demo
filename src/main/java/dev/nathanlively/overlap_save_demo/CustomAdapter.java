...
public class CustomAdapter implements Convolution {
    ...
    public double[] with(double[] signal, double[] kernel) {
        ...
        final double[] paddedSignal = preparePaddedSignal(signal, kernel.length);
        final double[] preparedKernel = prepareKernel(kernel);

        return computeConvolution(paddedSignal, preparedKernel, signal.length, kernel.length);
    }
    ...
    private double[] preparePaddedSignal(double[] signal, int kernelLength) {
    ...
    private double[] computeConvolution(double[] paddedSignal, double[] preparedKernel,
                                        int signalLength, int kernelLength) {
        ...
        for (int outputPos = 0; outputPos < resultLength; outputPos++) {
            result[outputPos] = computeWindowConvolution(paddedSignal, preparedKernel,
